...
async function fetchUserNameById(userId: number): Promise<string> {
  return bridge
    .send("VKWebAppGetUserInfo", { user_id: userId })
    .then((data) => {
      if (data.first_name) {
...
export const Game: React.FC<GameProps> = ({
  ...
  useEffect(() => {
    if (fetchedUser) {
      const ws = new WebSocket("ws://hshcode.ru:8080");
      setSocket(ws);
      ...
      ws.onmessage = (event) => {
        ...
        switch (message.type) {
          ...
          case "game_end":
            ...
            } else if (partyCount !== undefined) {
              // Здесь нужен проигравший
        ...
        {gameStarted ? (
          <>
            ...
            {currentPlayerId === userID?.toString() && (
              <InfoBoxOutlineDown
                time={3}
              >{`${userName} Ваш ход`}</InfoBoxOutlineDown>
            )}
            {currentPlayerId !== userID?.toString() && (
              <InfoBoxOutlineDown
                time={0}
              >{`Сейчас ходит ${opponentName}`}</InfoBoxOutlineDown>
            )}
          </>
        ) : (
